---
name: thesis-tracker
description: 投资论文追踪系统。用于买入后持续检查核心假设、证伪信号、估值锚点和仓位纪律。
---
...
当前日期为 `运行环境当前日期`。追踪检查必须基于截至今日已披露的最新财报和事件，搜索query中必须包含当前年份。
...
使用 网页搜索/浏览工具 获取当前股价、估值指标（PE/PB/股息率）、最新财报核心数据，用于填写估值锚点。如果已有该公司的 `/investment-research` 或 `/investment-team` 报告，优先从中读取。

使用 `skills/financial-data/scripts/financial_rigor.py verify-valuation` 校验估值数据。
