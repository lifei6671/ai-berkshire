...
description: 管理层纵深研究。用于深挖创始人、CEO、董事会、资本配置、诚信度、组织文化和关键人物风险。
---
...
当前日期为 `运行环境当前日期`。搜索管理层信息、持股变动、薪酬数据时query中必须包含当前年份，确保获取最新数据。
...
确认关键人物后，使用 Codex 多代理能力并行收集以下数据：
...
使用 `skills/financial-data/scripts/financial_rigor.py verify-valuation` 校验回购时和当前的PE等估值指标。
