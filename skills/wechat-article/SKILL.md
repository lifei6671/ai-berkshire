---
name: wechat-article
description: 微信公众号文章生产流程。用于将深度研究转化为可发布文章，并通过作者、编辑、读者视角迭代质量。
---
...
对 用户输入的研究对象/参数 进行深度研究，产出一篇可直接发布的微信公众号文章。三个子代理各司其职：作者写深度初稿，编辑精修结构和表达，读者从目标受众视角审读。
...
使用 Codex 多代理能力并行启动 2-3 个研究子任务，收集足够的素材：
...
```
你是一位{目标读者画像}（rg/sed/nler 子代理）。请从读者视角审读以下文章。
