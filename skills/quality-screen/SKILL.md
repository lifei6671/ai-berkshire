---
name: quality-screen
description: 去劣筛选流程。用于按 7 条硬指标快速排除非一流公司，支持个股、行业、指数或主题批量筛选。
---
...
对 用户输入的研究对象/参数 执行去劣指标筛选，快速排除不符合一流公司标准的标的。
...
当前日期为 `运行环境当前日期`。筛选指标必须基于截至今日已披露的最新财年数据，搜索query中必须包含当前年份。
...
- 如果输入是行业/市场/主题 → **批量模式**，先执行以下操作：
  1. 用 网页搜索/浏览工具 搜索该行业/市场/主题下的主要上市公司
...
为每家公司启动独立后台子代理，使用 网页搜索/浏览工具 搜索以下数据：
