...
description: 为一家公司生成多篇深度长文系列。用于需要公众号级公司拆解、连续内容策划、认知重置和投资判断闭环的场景。
---
...
为 用户输入的研究对象/参数 撰写一个 8 篇深度长文系列，发布在公众号/视频号等公开渠道。**核心 IP 不是"会写"，而是"会改"——99% 的财经文章在违反本 skill 的事实核查标准**。
...
如果用户说"X 数据不对"，先用 shell_command/rg/sed/nl 找原始数据交叉验证：
...
- 推送前必须用 grep 扫描 `linxuan` / `/Users/` / 用户公司花名 等隐私字段（参见 `项目内隐私扫描规则`）
- 公开署名按用户多层身份策略，不混用
