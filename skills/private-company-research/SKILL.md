...
description: 未上市公司深度研究。用于研究 SpaceX、Stripe、蚂蚁、小红书等信息不完整公司的商业模式、估值和上市路径。
---
...
对 用户输入的研究对象/参数 进行团队化深度研究分析。专为蚂蚁集团、小红书、SpaceX、Stripe 等未上市公司设计。
...
当前日期为 `运行环境当前日期`。所有数据搜索和引用必须基于此日期，搜索query中必须包含当前年份。融资轮次、估值、收入数据须标注数据截至时间。
...
| 角色 | 职责 | 核心视角 |
|------|------|----------|
| **主代理**（你自己） | 统筹协调、信息拼图、交叉验证、输出最终报告 | 投资决策整合 |
...
规划 Codex 子代理批次：
- batch_name: `{公司名}-private-research`（英文小写，如 `ant-group-private-research`）
- lead_role: `主代理`

### 第三步：创建6个任务

记录以下6个任务（每个都要有 subject、description、任务显示名）：

---

#### 任务1：商业模式与产品用户深度分析
- subject: `拆解{公司名}商业模式、产品矩阵与用户生态`
- 任务显示名: `分析{公司名}商业模式与用户生态`
- description 包含：
...
**研究方法**：
1. 使用 网页搜索/浏览工具 搜索最新公开信息，每个维度至少搜索3-5次，用不同关键词组合
2. 搜索关键词策略：
...
4. 使用 网页打开/读取工具 获取关键文章的全文（不要只看搜索摘要）
...
1. **6个子代理必须并行启动**——在同一条消息中调用6次子代理工具
