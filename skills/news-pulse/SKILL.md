---
name: news-pulse
description: 公司新闻脉搏。用于股价异动或突发新闻时快速归因，侦察公司事件、监管政策、行业对手和市场情绪。
---
...
对 用户输入的研究对象/参数 进行最近新闻侦察与异动归因。**这不是深度投研，是情报快速响应**——目标是 10 分钟内回答："这家公司最近发生了什么？股价异动的真因是什么？要不要重审投资论文？"

**日期锚定**：当前日期为 `运行环境当前日期`。所有新闻搜索必须聚焦最近1-4周，query中带当前年月。
...
向用户澄清以下信息（如未在 用户输入的研究对象/参数 中提供）：
...
参考 `skills/investment-team/SKILL.md` 的 A/B/C 评级，但维度不同：

| 等级 | 特征 | 侦察策略 |
|------|------|---------|
| **A 级（信息充裕）** | 大盘股、媒体覆盖广、财报季 | 重点是**降噪和归因**——信息太多反而难找真因，每个 子代理 要有判断力，过滤掉"重复转述"的二手新闻 |
...
规划 Codex 子代理批次：
- `batch_name`: `{公司名}-newspulse`（英文小写，如 `pdd-newspulse`）
- `lead_role`: `主代理`
...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 skills/news-pulse/scripts/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     - 段永平 user_id: `1247347556`
     - 命令示例：`python3 skills/news-pulse/scripts/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/dyp-{公司名}.md`
...
并行派发 4 个 Codex 子任务，每个子任务需包含：
- `role`: `general-purpose`
- `并行执行`: `true`
- `batch_name`: `{公司名}-newspulse`
...
**侦察方法**：
- 优先使用 网页搜索/浏览工具 搜索时效性查询（关键词加日期或"最近"、"latest"、"2026"）
- 关键事件用 网页打开/读取工具 精读原始来源（公告原文、财报、监管文件）
...
**完成后**：
1. 在主代理进度清单中将任务标记为 completed
2. 将完整侦察报告作为结构化结果返回给主代理
```
