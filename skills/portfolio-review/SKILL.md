...
description: 投资组合审视与优化。用于分析持仓结构、集中度、风险暴露、再平衡和投资论文一致性。
---
...
当前日期为 `运行环境当前日期`。估值、股价、持仓市值必须基于最近交易日数据，搜索时query带当前年份。
...
使用 Codex 多代理能力，通过网页搜索/浏览工具为每个持仓并行获取：
1. 当前股价和估值指标（PE、PB、股息率）
2. 最近一个季度的关键财务变化
3. 近期重大事件
4. 分析师一致预期（前瞻PE、目标价）

对每个持仓使用 `skills/financial-data/scripts/financial_rigor.py verify-valuation` 校验估值数据。对每只持仓标注信息丰富度（A/B/C级），C级持仓的分析结论标注低置信度。
...
预期回报估算方法（使用 `skills/financial-data/scripts/financial_rigor.py three-scenario` 计算）：
