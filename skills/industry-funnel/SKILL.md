...
description: 行业漏斗筛选流程。用于从行业、主题或市场方向中全市场粗筛、逐层排除并选出少数值得深研的标的。
---
...
对 用户输入的研究对象/参数 行业/方向执行漏斗式价值投资筛选，从全市场扫描逐层精选到 3 家终选标的。
...
**日期锚定**：当前日期为 `运行环境当前日期`。筛选数据必须基于截至今日已披露的最新财年，搜索query中必须包含当前年份。
